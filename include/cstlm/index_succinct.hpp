...
public:
    ...
    uint64_t m_N1PlusBack(const node_type& node, uint64_t pattern_begin_symbol) const
    {
        ...
        if (pattern_begin_symbol == PAT_START_SYM) {
    ...
    uint64_t m_N1PlusFrontBack(const node_type& node,
        uint64_t pattern_begin,
        uint64_t pattern_end,
        size_t pattern_size) const
    {
        ...
        if (!m_cst.is_leaf(node) && pattern_size == m_cst.depth(node)) {
            ...
            if (pattern_begin == PAT_START_SYM && pattern_end == PAT_END_SYM) {
                ...
                return m_N1PlusBack(node, pattern_begin);
            }
        }
    }
